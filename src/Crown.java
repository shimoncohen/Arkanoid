...
public class Crown implements Sprite {
    ...
    static final int TRIANGLE_SIZE = (Ass5Game.SCREEN_WIDTH - 2 * DISTANCE_FROM_EDGES) / 4;
    ...
    public void drawOn(DrawSurface d) {
        ...
        d.fillRectangle(DISTANCE_FROM_EDGES, DISTANCE_FROM_EDGES, Ass5Game.SCREEN_WIDTH - 2 * DISTANCE_FROM_EDGES,
                Ass5Game.SCREEN_HEIGHT - 200);
        ...
        for (int i = 0; i < 15; i++) {
            ...
            p2.addPoint(x, DISTANCE_FROM_EDGES + (Ass5Game.SCREEN_HEIGHT - 2 * DISTANCE_FROM_EDGES)
                    / 2 + 50);
            p2.addPoint(x - GEM_SIZE / 2, DISTANCE_FROM_EDGES
                    + (Ass5Game.SCREEN_HEIGHT - 2 * DISTANCE_FROM_EDGES) / 2 + 50 + GEM_SIZE);
            p2.addPoint(x, DISTANCE_FROM_EDGES
                    + (Ass5Game.SCREEN_HEIGHT - 2 * DISTANCE_FROM_EDGES) / 2 + 50 + 2 * GEM_SIZE);
            p2.addPoint(x + GEM_SIZE / 2, DISTANCE_FROM_EDGES
                    + (Ass5Game.SCREEN_HEIGHT - 2 * DISTANCE_FROM_EDGES) / 2 + 50 + GEM_SIZE);
